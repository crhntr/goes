...
func NewEventSource(uri string) (*EventSource, error) {
	esv := js.Global().Get("EventSource").New(uri)
...
func (es *EventSource) Handle(eventName string, handler ServerEventHandlerFunc) {
	es.Call("addEventListener", eventName, js.FuncOf(func(_ js.Value, args []js.Value) interface{} {
		...
		if es.LogEvents {
			js.Global().Get("console").Call("log", "EventSource", id, js.Global().Get("JSON").Call("parse", string(data)))
		}
