...
// Title is a page option function
func Title(str string) func(*Page) {
	return func(pg *Page) {
		pg.Title = str
	}
}

// Template is a page option function
func Template(str string) func(*Page) {
	return func(pg *Page) {
		pg.Template = str
	}
}

// GoJSSource is a page option function
func GoJSSource(str string) func(*Page) {
	return func(pg *Page) {
		pg.GoJS = str
	}
}

// WASMBin is a page option function
func WASMBin(str string) func(*Page) {
	return func(pg *Page) {
		pg.WASMBin = str
	}
}

// HTMLPage renders HTML page to io.Writer.
func HTMLPage(w io.Writer, opts ...PageOption) error {
	var page Page
	...
	if page.GoJS == "" {
		page.GoJS = MinimalIndexPageExecutableWASMPath
	}
	if page.WASMBin == "" {
		page.WASMBin = MinimalIndexPageGoWASMExecPath
	}

	tmpl, err := template.New(page.Title).Parse(page.Template)
	if err != nil {
		return err
	}
...
<html lang="en" dir="ltr">
	<head>
		...
		<script>
			...
			const go = new Go();
			WebAssembly.instantiateStreaming(fetch("{{ .WASMBin }}"), go.importObject).then((result) => {
